...
namespace RIMAPI.Services
{
    public class PawnEditService : IPawnEditService
    {
        ...
        // --- 2. Health (Heal, Disease, Body Parts) ---
        ...
        public ApiResult UpdateSkills(PawnSkillsRequest request)
        {
            try
            {
                ...
                foreach (var skillDto in request.Skills)
                {
                    ...
                    if (!string.IsNullOrEmpty(skillDto.Passion) && Enum.TryParse(skillDto.Passion, true, out Passion p))
                    {
                        record.passion = p;
                    }
                }
