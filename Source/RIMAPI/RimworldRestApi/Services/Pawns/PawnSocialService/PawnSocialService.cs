    ...
    public class PawnSocialService : IPawnSocialService
    {
        ...
        public ApiResult<PawnInteractionStatusDto> GetPawnInteractionStatus(int pawnId)
        {
            try
            {
                ...
                // Fix: use InteractedTooRecentlyToInteract() method
                ...
                var status = new PawnInteractionStatusDto
                {
                    ...
                    CooldownDays = cooldownTicks.TicksToDays()
                };
        ...
        public ApiResult<PawnInteractionLogDto> GetPawnInteractionLog(int pawnId)
        {
            try
            {
                var pawn = PawnHelper.FindPawnById(pawnId);
                ...
                foreach (var entry in playLog.AllEntries)
                {
                    ...
                    if (entry is PlayLogEntry_Interaction interaction)
                    {
                        ...
                        if (isInit || isRec)
                        {
                            ...
                            // CRITICAL FIX Restored: Use the exact memory reference for POV to prevent game engine string crashes
                ...
                var orderedInteractions = logEntries.OrderByDescending(e => e.Ticks).ToList();
                        ...
                        if (pawn.RaceProps.Humanlike && otherPawn.RaceProps.Humanlike)
                        {
                            ...
                            if (opinion >= 20)
                            {
                                result.Relations.Add(new PawnRelationEntryDto
                                {
                                    OtherPawnId = otherPawn.thingIDNumber,
                                    OtherPawnName = otherPawn.LabelShort,
                                    RelationDefName = "Friend", // Pseudo-def for API consumption
                                    RelationLabel = "Friend".Translate() // Uses the game's localized string
                                });
                            }
                            else if (opinion <= -20)
                            {
                                result.Relations.Add(new PawnRelationEntryDto
                                {
                                    OtherPawnId = otherPawn.thingIDNumber,
                                    OtherPawnName = otherPawn.LabelShort,
                                    RelationDefName = "Rival", // Pseudo-def
            ...
            try
            {
                ...
                foreach (var otherPawn in allPawns)
                {
                    ...
                    if (pawn.needs?.mood?.thoughts != null)
                    {
                        ...
                        {
                            if (t is Thought_SituationalSocial ss && ss.OtherPawn == otherPawn)
                            {
                                opinionDto.Breakdown.Add(new OpinionBreakdownDto
                                {
                                    ThoughtDefName = t.def.defName,
                                    Label = t.LabelCap,
                        ...
                        // Memories
                        foreach (var t in pawn.needs.mood.thoughts.memories.Memories)
                        {
                            if (t is Thought_MemorySocial ms && ms.OtherPawn == otherPawn)
                            {
                                opinionDto.Breakdown.Add(new OpinionBreakdownDto
                                {
                                    ThoughtDefName = t.def.defName,
