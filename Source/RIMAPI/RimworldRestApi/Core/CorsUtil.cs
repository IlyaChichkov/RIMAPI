...
namespace RimworldRestApi.Core
{
    public static class CorsUtil
    {
        public static void WriteCors(HttpListenerRequest req, HttpListenerResponse res, ISet<string> allowedOrigins = null)
        {
            ...
            res.Headers.Set("Access-Control-Allow-Headers", "Content-Type, Accept, Authorization, ETag, If-None-Match");
        }

        public static void WritePreflight(HttpListenerContext ctx, IEnumerable<string> methods = null, IEnumerable<string> extraHeaders = null)
        {
            DebugLogging.Info($"WritePreflight");
            var res = ctx.Response;

            res.Headers.Set("Access-Control-Allow-Methods",
                methods != null ? string.Join(", ", methods) : "GET, POST, PUT, PATCH, DELETE, OPTIONS");
