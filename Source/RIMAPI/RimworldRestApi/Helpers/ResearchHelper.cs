...
namespace RimworldRestApi.Helpers
{
    public class ResearchHelper
    {
        ...
        private ResearchProjectDto ResearchProjectToDto(ResearchProjectDef project)
        {
            try
            {
                if (project == null)
                ...
                return new ResearchProjectDto
                {
                    ...
                    PlayerHasAnyAppropriateResearchBench = project.PlayerHasAnyAppropriateResearchBench,
                    ...
                    Prerequisites = project.prerequisites?.Select(p => p.defName).ToList() ?? new List<string>(),
                    HiddenPrerequisites = project.hiddenPrerequisites?.Select(p => p.defName).ToList() ?? new List<string>(),
                    RequiredByThis = project.requiredByThis?.Select(p => p.defName).ToList() ?? new List<string>(),
        ...
        public ResearchSummaryDto GetResearchSummary()
        {
            try
            {
                ...
                if (storyteller?.difficulty == null)
                    throw new InvalidOperationException("Storyteller or difficulty values are not available");
                ...
                if (allResearchProjects == null)
                    throw new InvalidOperationException("Research projects database is not available");

                foreach (ResearchProjectDef projectDef in allResearchProjects)
                {
                    if (projectDef == null) continue;
